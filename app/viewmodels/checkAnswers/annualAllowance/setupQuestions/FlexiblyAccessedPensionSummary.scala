...
object FlexiblyAccessedPensionSummary {

  def row(answers: UserAnswers)(implicit messages: Messages): Option[SummaryListRow] =
    answers.get(FlexiblyAccessedPensionPage).map { answer =>
      ...
      SummaryListRowViewModel(
        ...
        actions = Seq(
          ActionItemViewModel("site.change", routes.FlexiblyAccessedPensionController.onPageLoad(CheckMode).url)
            .withVisuallyHiddenText(messages("flexiblyAccessedPension.change.hidden"))
        )
      )
    }
}
