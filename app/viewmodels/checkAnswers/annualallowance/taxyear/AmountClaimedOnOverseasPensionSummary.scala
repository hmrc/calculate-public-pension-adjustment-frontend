  ...
  def row(answers: UserAnswers, period: Period)(implicit messages: Messages): Option[SummaryListRow] =
    answers.get(AmountClaimedOnOverseasPensionPage(period)).map { answer =>
      ...
      SummaryListRowViewModel(
        key = messages("amountClaimedOnOverseasPension.checkYourAnswersLabel", startEndDate),
        ...
        actions = Seq(
          ActionItemViewModel(
            ...
            .withVisuallyHiddenText(messages("amountClaimedOnOverseasPension.change.hidden", startEndDate))
        )
      )
    }
}
