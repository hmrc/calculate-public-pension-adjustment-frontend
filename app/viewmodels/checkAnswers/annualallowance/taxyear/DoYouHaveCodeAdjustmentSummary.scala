...
object DoYouHaveCodeAdjustmentSummary {

  def row(answers: UserAnswers, period: Period)(implicit messages: Messages): Option[SummaryListRow] =
    answers.get(DoYouHaveCodeAdjustmentPage(period)).map { answer =>
      ...
      SummaryListRowViewModel(
        key = messages("doYouHaveCodeAdjustment.checkYourAnswersLabel", startEndDate),
        value = ValueViewModel(value),
        actions = Seq(
          ActionItemViewModel(
            "site.change",
            controllers.annualallowance.taxyear.routes.DoYouHaveCodeAdjustmentController.onPageLoad(CheckMode, period).url
          )
