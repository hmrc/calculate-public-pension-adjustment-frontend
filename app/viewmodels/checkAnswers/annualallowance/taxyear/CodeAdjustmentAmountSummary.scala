...
object CodeAdjustmentAmountSummary {
  ...
  def row(answers: UserAnswers, period: Period)(implicit
    ...
    answers.get(CodeAdjustmentAmountPage(period)).map { answer =>
      SummaryListRowViewModel(
        key = messages("codeAdjustmentAmount.checkYourAnswersLabel", startEndDate),
