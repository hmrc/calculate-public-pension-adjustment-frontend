...
package viewmodels.checkAnswers.annualallowance
...
object PensionSchemeDetailsSummary {
  ...
  def row(answers: UserAnswers, period: Period, schemeIndex: SchemeIndex)(implicit
    ...
    answers.get(PensionSchemeDetailsPage(period, schemeIndex)).map { answer =>
      val value =
        HtmlFormat.escape(answer.schemeName).toString + " / " + HtmlFormat.escape(answer.schemeTaxRef).toString
