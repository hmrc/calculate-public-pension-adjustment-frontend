...
object ScottishTaxpayerFrom2016Summary {
  ...
  def row(answers: UserAnswers)(implicit messages: Messages): Option[SummaryListRow] =
    answers.get(ScottishTaxpayerFrom2016Page).map { answer =>
      ...
      SummaryListRowViewModel(
        ...
        actions = Seq(
          ActionItemViewModel("site.change", routes.ScottishTaxpayerFrom2016Controller.onPageLoad(CheckMode).url)
            .withVisuallyHiddenText(messages("scottishTaxpayerFrom2016.change.hidden"))
        )
      )
    }
}
