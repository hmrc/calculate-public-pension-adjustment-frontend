...
object PayingPublicPensionSchemeSummary {

  def row(answers: UserAnswers)(implicit messages: Messages): Option[SummaryListRow] =
    answers.get(PayingPublicPensionSchemePage).map { answer =>
      ...
      SummaryListRowViewModel(
        ...
        actions = Seq(
          ActionItemViewModel("site.change", routes.PayingPublicPensionSchemeController.onPageLoad(CheckMode).url)
            .withVisuallyHiddenText(messages("payingPublicPensionScheme.change.hidden"))
        )
      )
    }
}
