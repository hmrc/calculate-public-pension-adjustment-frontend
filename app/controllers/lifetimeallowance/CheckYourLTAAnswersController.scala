...
class CheckYourLTAAnswersController @Inject() (
  ...
  def onPageLoad(): Action[AnyContent] = (identify andThen getData andThen requireData) { implicit request =>
    val rows: Seq[Option[SummaryListRow]] = Seq(
      ...
      NewEnhancementTypeSummary.row(request.userAnswers),
      NewInternationalEnhancementReferenceSummary.row(request.userAnswers),
      NewPensionCreditReferenceSummary.row(request.userAnswers),
