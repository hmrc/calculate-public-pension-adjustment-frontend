...
class NewPensionCreditReferenceController @Inject() (
  ...
  def onPageLoad(mode: Mode): Action[AnyContent] = (identify andThen getData andThen requireData) { implicit request =>
    val preparedForm = request.userAnswers.get(NewPensionCreditReferencePage) match {
      case None        => form
    ...
    implicit request =>
      form
        ...
        .fold(
          ...
          value =>
            for {
              updatedAnswers <- Future.fromTry(request.userAnswers.set(NewPensionCreditReferencePage, value))
              redirectUrl     = NewPensionCreditReferencePage.navigate(mode, updatedAnswers).url
