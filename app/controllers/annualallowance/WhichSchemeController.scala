...
import forms.annualallowance.WhichSchemeFormProvider
import models.{Mode, Period, SchemeIndex}
import pages.annualallowance.WhichSchemePage
import play.api.i18n.{I18nSupport, MessagesApi}
import play.api.mvc.{Action, AnyContent, MessagesControllerComponents}
import repositories.SessionRepository
import services.SchemeService
import uk.gov.hmrc.play.bootstrap.frontend.controller.FrontendBaseController
import views.html.annualallowance.WhichSchemeView
...
class WhichSchemeController @Inject() (
  ...
  formProvider: WhichSchemeFormProvider,
  val controllerComponents: MessagesControllerComponents,
  view: WhichSchemeView
  ...
  def onPageLoad(mode: Mode, period: Period, schemeIndex: SchemeIndex): Action[AnyContent] =
    (identify andThen getData andThen requireData) { implicit request =>
      val preparedForm = request.userAnswers.get(WhichSchemePage(period, schemeIndex)) match {
        case None        => form
        case Some(value) => form.fill(value)
      }

      Ok(view(preparedForm, mode, period, schemeIndex, SchemeService.whichScheme(request.userAnswers)))
    }
      ...
      form
        ...
        .fold(
          formWithErrors =>
            Future.successful(
              BadRequest(
                view(formWithErrors, mode, period, schemeIndex, SchemeService.whichScheme(request.userAnswers))
              )
            ),
          value => {
