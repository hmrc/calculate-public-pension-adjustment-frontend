...
class AmountClaimedOnOverseasPensionController @Inject()(
                                        override val messagesApi: MessagesApi,
                                        userDataService: UserDataService,
                                        identify: IdentifierAction,
                                        getData: DataRetrievalAction,
                                        requireData: DataRequiredAction,
                                        formProvider: AmountClaimedOnOverseasPensionFormProvider,
                                        val controllerComponents: MessagesControllerComponents,
                                        view: AmountClaimedOnOverseasPensionView
  ...
  def onPageLoad(mode: Mode, period: Period): Action[AnyContent] = (identify andThen getData andThen requireData) {
    implicit request =>
      ...
      val preparedForm = request.userAnswers.get(AmountClaimedOnOverseasPensionPage(period)) match {
        case None => form
  ...
  def onSubmit(mode: Mode, period: Period): Action[AnyContent] = (identify andThen getData andThen requireData).async {
    implicit request =>
      ...
      form.bindFromRequest().fold(
        ...
        value =>
        for {
          updatedAnswers <- Future.fromTry(request.userAnswers.set(AmountClaimedOnOverseasPensionPage(period), value))
          redirectUrl = AmountClaimedOnOverseasPensionPage(period).navigate(mode, updatedAnswers).url
