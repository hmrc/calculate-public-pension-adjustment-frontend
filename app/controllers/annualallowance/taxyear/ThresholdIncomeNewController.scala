...
import views.html.annualallowance.taxyear.ThresholdIncomeNewView

import javax.inject.Inject
import scala.concurrent.{ExecutionContext, Future}

class ThresholdIncomeNewController @Inject() (
  override val messagesApi: MessagesApi,
  userDataService: UserDataService,
  identify: IdentifierAction,
  getData: DataRetrievalAction,
  requireData: DataRequiredAction,
  formProvider: ThresholdIncomeNewFormProvider,
  val controllerComponents: MessagesControllerComponents,
  view: ThresholdIncomeNewView
  ...
  def onPageLoad(mode: Mode, period: Period): Action[AnyContent] = (identify andThen getData andThen requireData) {
    implicit request =>
      ...
      val preparedForm = request.userAnswers.get(ThresholdIncomeNewPage(period)) match {
        case None        => form
    ...
    implicit request =>
      ...
      form
        ...
        .fold(
          ...
          value =>
            for {
              updatedAnswers <- Future.fromTry(request.userAnswers.set(ThresholdIncomeNewPage(period), value))
              redirectUrl     = ThresholdIncomeNewPage(period).navigate(mode, updatedAnswers).url
