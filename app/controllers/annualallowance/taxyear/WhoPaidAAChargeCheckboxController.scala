...
import pages.annualallowance.taxyear.{PensionSchemeDetailsPage, WhoPaidAAChargeCheckboxPage}
import play.api.i18n.{I18nSupport, Messages, MessagesApi}
import play.api.mvc.{Action, AnyContent, MessagesControllerComponents}
import services.UserDataService
import uk.gov.hmrc.play.bootstrap.frontend.controller.FrontendBaseController
import views.html.annualallowance.taxyear.WhoPaidAAChargeCheckboxView
...
class WhoPaidAAChargeCheckboxController @Inject()(
                                        override val messagesApi: MessagesApi,
                                        userDataService: UserDataService,
                                        identify: IdentifierAction,
                                        getData: DataRetrievalAction,
                                        requireData: DataRequiredAction,
                                        formProvider: WhoPaidAAChargeCheckboxFormProvider,
                                        val controllerComponents: MessagesControllerComponents,
                                        view: WhoPaidAAChargeCheckboxView
  ...
  def onPageLoad(mode: Mode, period: Period, schemeIndex: SchemeIndex): Action[AnyContent] = (identify andThen getData andThen requireData) {
    implicit request =>
      ...
      val preparedForm = request.userAnswers.get(WhoPaidAAChargeCheckboxPage(period, schemeIndex)) match {
        case None => form
  ...
  def onSubmit(mode: Mode, period: Period, schemeIndex: SchemeIndex): Action[AnyContent] = (identify andThen getData andThen requireData).async {
    implicit request =>
      ...
      form.bindFromRequest().fold(
        formWithErrors =>
          Future.successful(BadRequest(view(formWithErrors, mode, period, schemeIndex, schemeName.getOrElse(""), startEndDate(period)))),

        value =>
          for {
            updatedAnswers <- Future.fromTry(request.userAnswers.set(WhoPaidAAChargeCheckboxPage(period, schemeIndex), value))
            redirectUrl     = WhoPaidAAChargeCheckboxPage(period, schemeIndex).navigate(mode, updatedAnswers).url
