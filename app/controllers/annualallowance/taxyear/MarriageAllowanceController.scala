...
class MarriageAllowanceController @Inject()(
  ...
  def onPageLoad(mode: Mode, period: Period): Action[AnyContent] = (identify andThen getData andThen requireData) {
    implicit request =>
      ...
      val preparedForm = request.userAnswers.get(MarriageAllowancePage(period)) match {
        case None => form
  ...
  def onSubmit(mode: Mode, period: Period): Action[AnyContent] = (identify andThen getData andThen requireData).async {
    implicit request =>
      ...
      form.bindFromRequest().fold(
        ...
        value =>
          for {
            updatedAnswers <- Future.fromTry(request.userAnswers.set(MarriageAllowancePage(period), value))
            redirectUrl = MarriageAllowancePage(period).navigate(mode, updatedAnswers).url
