...
import forms.annualallowance.taxyear.HowMuchAAChargePrivateSchemePaidFormProvider
import models.tasklist.sections.AASection
import models.{Mode, Period, SchemeIndex}
import pages.annualallowance.taxyear.HowMuchAAChargePrivateSchemePaidPage
import play.api.i18n.{I18nSupport, Messages, MessagesApi}
import play.api.mvc.{Action, AnyContent, MessagesControllerComponents}
import services.UserDataService
import uk.gov.hmrc.play.bootstrap.frontend.controller.FrontendBaseController
import views.html.annualallowance.taxyear.HowMuchAAChargePrivateSchemePaidView
...
class HowMuchAAChargePrivateSchemePaidController @Inject()(
  override val messagesApi: MessagesApi,
  userDataService: UserDataService,
  identify: IdentifierAction,
  getData: DataRetrievalAction,
  requireData: DataRequiredAction,
  formProvider: HowMuchAAChargePrivateSchemePaidFormProvider,
  val controllerComponents: MessagesControllerComponents,
  view: HowMuchAAChargePrivateSchemePaidView
  ...
  def onPageLoad(mode: Mode, period: Period, schemeIndex: SchemeIndex): Action[AnyContent] =
    (identify andThen getData andThen requireData) { implicit request =>
      ...
      val preparedForm = request.userAnswers.get(HowMuchAAChargePrivateSchemePaidPage(period, schemeIndex)) match {
        case None        => form
      ...
      form
        ...
        .fold(
          ...
          value =>
            for {
              updatedAnswers <-
                Future.fromTry(request.userAnswers.set(HowMuchAAChargePrivateSchemePaidPage(period, schemeIndex), value))
              redirectUrl     = HowMuchAAChargePrivateSchemePaidPage(period, schemeIndex).navigate(mode, updatedAnswers).url
