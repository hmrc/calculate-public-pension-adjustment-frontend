...
class AmountOfGiftAidController @Inject() (
  override val messagesApi: MessagesApi,
  userDataService: UserDataService,
  identify: IdentifierAction,
  getData: DataRetrievalAction,
  requireData: DataRequiredAction,
  formProvider: AmountOfGiftAidFormProvider,
  val controllerComponents: MessagesControllerComponents,
  view: AmountOfGiftAidView
  ...
  def onPageLoad(mode: Mode, period: Period): Action[AnyContent] = (identify andThen getData andThen requireData) {
    implicit request =>
      ...
      val preparedForm = request.userAnswers.get(AmountOfGiftAidPage(period)) match {
        case None        => form
    ...
    implicit request =>
      ...
      form
        ...
        .fold(
          ...
          value =>
            for {
              updatedAnswers <- Future.fromTry(request.userAnswers.set(AmountOfGiftAidPage(period), value))
              redirectUrl     = AmountOfGiftAidPage(period).navigate(mode, updatedAnswers).url
