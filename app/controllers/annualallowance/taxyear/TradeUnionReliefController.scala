...
class TradeUnionReliefController @Inject() (
  override val messagesApi: MessagesApi,
  userDataService: UserDataService,
  identify: IdentifierAction,
  getData: DataRetrievalAction,
  requireData: DataRequiredAction,
  formProvider: TradeUnionReliefFormProvider,
  val controllerComponents: MessagesControllerComponents,
  view: TradeUnionReliefView
  ...
  def onPageLoad(mode: Mode, period: Period): Action[AnyContent] = (identify andThen getData andThen requireData) {
    implicit request =>
      val preparedForm = request.userAnswers.get(TradeUnionReliefPage(period)) match {
        case None        => form
    ...
    implicit request =>
      form
        ...
        .fold(
          ...
          value =>
            for {
              updatedAnswers <- Future.fromTry(request.userAnswers.set(TradeUnionReliefPage(period), value))
              redirectUrl     = TradeUnionReliefPage(period).navigate(mode, updatedAnswers).url
