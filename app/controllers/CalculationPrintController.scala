...
import views.html.CalculationPrintView

import javax.inject.Inject
import scala.concurrent.ExecutionContext

class CalculationPrintController @Inject()(
  override val messagesApi: MessagesApi,
  identify: IdentifierAction,
  getData: DataRetrievalAction,
  requireData: DataRequiredAction,
  val controllerComponents: MessagesControllerComponents,
  view: CalculationPrintView,
  ...
  def onPageLoad: Action[AnyContent] = (identify andThen getData andThen requireData).async { implicit request =>
    calculationResultService.sendRequest(request.userAnswers).map { calculationResponse =>
      ...
      Ok(
        view(
          form,
          calculationResultService.calculationPrintViewModel(calculationResponse),
