...
import models.{AAKickOutStatus, MaybePIAUnchangedOrDecreased, Mode}
...
class MaybePIAUnchangedOrDecreasedController @Inject() (
  override val messagesApi: MessagesApi,
  userDataService: UserDataService,
  identify: IdentifierAction,
  getData: DataRetrievalAction,
  requireData: DataRequiredAction,
  formProvider: MaybePIAUnchangedOrDecreasedFormProvider,
  val controllerComponents: MessagesControllerComponents,
  view: MaybePIAUnchangedOrDecreasedView
  ...
  def onPageLoad(mode: Mode): Action[AnyContent] = (identify andThen getData andThen requireData) { implicit request =>
    val preparedForm = request.userAnswers.get(MaybePIAUnchangedOrDecreasedPage) match {
      case None        => form
    ...
    implicit request =>
      form
        ...
        .fold(
          ...
          value => {
            val aaKickOutStatus =
              if (value == MaybePIAUnchangedOrDecreased.Yes) kickOutStatusFalse else kickOutStatusCompleted
            for {
              updatedAnswers          <- Future.fromTry(request.userAnswers.set(MaybePIAUnchangedOrDecreasedPage, value))
              updatedAnswersWithStatus = AAKickOutStatus().saveAAKickOutStatus(updatedAnswers, aaKickOutStatus)
              redirectUrl              = MaybePIAUnchangedOrDecreasedPage.navigate(mode, updatedAnswersWithStatus).url
