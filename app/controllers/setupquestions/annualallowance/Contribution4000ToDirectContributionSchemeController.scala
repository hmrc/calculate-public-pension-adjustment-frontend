...
class Contribution4000ToDirectContributionSchemeController @Inject() (
  ...
  formProvider: Contribution4000ToDirectContributionSchemeFormProvider,
  val controllerComponents: MessagesControllerComponents,
  view: Contribution4000ToDirectContributionSchemeView
  ...
  def onPageLoad(mode: Mode): Action[AnyContent] = (identify andThen getData andThen requireData) { implicit request =>
    val preparedForm = request.userAnswers.get(Contribution4000ToDirectContributionSchemePage) match {
      case None        => form
    ...
    implicit request =>
      form
        ...
        .fold(
          ...
          value => {
            val aaKickOutStatus = if (value) kickOutStatusCompleted else kickOutStatusTrue
            for {
              updatedAnswers          <-
                Future.fromTry(request.userAnswers.set(Contribution4000ToDirectContributionSchemePage, value))
              updatedAnswersWithStatus = AAKickOutStatus().saveAAKickOutStatus(updatedAnswers, aaKickOutStatus)
              redirectUrl              = Contribution4000ToDirectContributionSchemePage.navigate(mode, updatedAnswersWithStatus).url
