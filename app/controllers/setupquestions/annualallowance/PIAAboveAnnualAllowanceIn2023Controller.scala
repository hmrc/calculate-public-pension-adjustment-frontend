...
class PIAAboveAnnualAllowanceIn2023Controller @Inject() (
  override val messagesApi: MessagesApi,
  userDataService: UserDataService,
  identify: IdentifierAction,
  getData: DataRetrievalAction,
  requireData: DataRequiredAction,
  formProvider: PIAAboveAnnualAllowanceIn2023FormProvider,
  val controllerComponents: MessagesControllerComponents,
  view: PIAAboveAnnualAllowanceIn2023View
  ...
  def onPageLoad(mode: Mode): Action[AnyContent] = (identify andThen getData andThen requireData) { implicit request =>
    val preparedForm = request.userAnswers.get(PIAAboveAnnualAllowanceIn2023Page) match {
      case None        => form
      ...
      form
        ...
        .fold(
          ...
          value => {
            ...
            for {
              updatedAnswers          <- Future.fromTry(request.userAnswers.set(PIAAboveAnnualAllowanceIn2023Page, value))
              updatedAnswersWithStatus = AAKickOutStatus().saveAAKickOutStatus(updatedAnswers, aaKickOutStatus)
              redirectUrl              = PIAAboveAnnualAllowanceIn2023Page.navigate(mode, updatedAnswersWithStatus).url
