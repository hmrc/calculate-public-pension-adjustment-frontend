...
object ContributedToDuringRemedyPeriod extends Enumerable.Implicits {

  case object Definedcontribution extends WithName("definedContribution") with ContributedToDuringRemedyPeriod
  ...
  def checkboxItems(implicit messages: Messages): Seq[CheckboxItem] =
    values.zipWithIndex.map { case (value, index) =>
      CheckboxItemViewModel(
        content = Text(messages(s"contributedToDuringRemedyPeriod.${value.toString}")),
        fieldId = "value",
        index = index,
