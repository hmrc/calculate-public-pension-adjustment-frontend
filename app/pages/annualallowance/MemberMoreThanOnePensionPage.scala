...
case class MemberMoreThanOnePensionPage(period: Period) extends QuestionPage[Boolean] {
  ...
  override def toString: String = "memberMoreThanOnePension"

  override protected def navigateInNormalMode(answers: UserAnswers): Call = {
    val firstPeriod = PeriodService.isFirstPeriod(answers, period)
    if (firstPeriod) {
      controllers.annualallowance.routes.PensionSchemeDetailsController.onPageLoad(NormalMode, period, SchemeIndex(0))
    } else controllers.annualallowance.routes.WhichSchemeController.onPageLoad(NormalMode, period, SchemeIndex(0))
  }
