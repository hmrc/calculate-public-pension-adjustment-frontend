...
import models.{CheckMode, MaybePIAIncrease, UserAnswers}
      ...
      MaybePIAIncreaseSummary.row(userAnswers) `shouldBe` Some(
        SummaryListRowViewModel(
          ...
          actions = Seq(
            ActionItemViewModel(
              "site.change",
              routes.MaybePIAIncreaseController.onPageLoad(CheckMode).url
            )
              .withVisuallyHiddenText("maybePIAIncrease.change.hidden")
          )
        )
      )
    }
