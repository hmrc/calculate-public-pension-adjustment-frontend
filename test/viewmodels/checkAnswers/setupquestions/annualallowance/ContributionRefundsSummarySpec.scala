      ...
      ContributionRefundsSummary.row(userAnswers) shouldBe Some(
        SummaryListRowViewModel(
          ...
          actions = Seq(
            ActionItemViewModel(
              "site.change",
              controllers.setupquestions.annualallowance.routes.ContributionRefundsController.onPageLoad(CheckMode).url
            )
              .withVisuallyHiddenText("contributionRefunds.change.hidden")
          )
        )
      )
    }
