      ...
      FlexibleRemunerationArrangementsSummary.row(userAnswers, period) shouldBe Some(
        SummaryListRowViewModel(
          ...
          actions = Seq(
            ActionItemViewModel(
              "site.change",
              routes.FlexibleRemunerationArrangementsController.onPageLoad(CheckMode, period).url
            )
              .withVisuallyHiddenText("flexibleRemunerationArrangements.change.hidden")
          )
        )
      )
    }
