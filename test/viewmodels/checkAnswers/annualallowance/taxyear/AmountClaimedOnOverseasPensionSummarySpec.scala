...
class AmountClaimedOnOverseasPensionSummarySpec extends AnyFreeSpec with Matchers {
  ...
  "row" - {
    "when value is entered, return the summary row" in {
      ...
      val userAnswers = UserAnswers("id")
        .set(
          AmountClaimedOnOverseasPensionPage(period),
          BigInt("100")
        )
        .get
      AmountClaimedOnOverseasPensionSummary.row(userAnswers, period) shouldBe Some(
        SummaryListRowViewModel(
          ...
          actions = Seq(
            ActionItemViewModel(
              "site.change",
              routes.AmountClaimedOnOverseasPensionController.onPageLoad(CheckMode, period).url
            )
              .withVisuallyHiddenText("amountClaimedOnOverseasPension.change.hidden")
          )
        )
      )
    }
