  ...
  "row" - {
    "when Yes is selected, return the summary row" in {
      val userAnswers = UserAnswers("id")
        ...
        .get
      MultipleBenefitCrystallisationEventSummary.row(userAnswers) shouldBe Some(
        SummaryListRowViewModel(
          ...
          actions = Seq(
            ActionItemViewModel(
              "site.change",
              routes.MultipleBenefitCrystallisationEventController.onPageLoad(CheckMode).url
            )
              .withVisuallyHiddenText("multipleBenefitCrystallisationEvent.change.hidden")
          )
        )
      )
    }
    ...
    "when answer unavailable, return empty" in {
      val userAnswers = UserAnswers("id")
      MultipleBenefitCrystallisationEventSummary.row(userAnswers) shouldBe None
    }
  }
