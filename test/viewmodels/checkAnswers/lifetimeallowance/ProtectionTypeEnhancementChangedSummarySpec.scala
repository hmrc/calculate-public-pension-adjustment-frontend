...
class ProtectionTypeEnhancementChangedSummarySpec extends AnyFreeSpec with Matchers {
  ...
  "row" - {
    "when Yes is selected, return the summary row" in {
      val userAnswers = UserAnswers("id")
        ...
        .get
      ProtectionTypeEnhancementChangedSummary.row(userAnswers) shouldBe Some(
        SummaryListRowViewModel(
          ...
          actions = Seq(
            ActionItemViewModel(
              ...
              .withVisuallyHiddenText("protectionTypeEnhancementChanged.change.hidden")
          )
        )
      )
    }
    ...
    "when answer unavailable, return empty" in {
      val userAnswers = UserAnswers("id")
      ProtectionTypeEnhancementChangedSummary.row(userAnswers) shouldBe None
    }
  }
}
