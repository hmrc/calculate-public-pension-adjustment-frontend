    ...
    "when value is entered, return the summary row and change link when true" in {
      ...
      NewPensionCreditReferenceSummary.row(userAnswers, true) shouldBe Some(
        SummaryListRowViewModel(
          ...
          actions = Seq(
            ActionItemViewModel("site.change", routes.NewPensionCreditReferenceController.onPageLoad(CheckMode).url)
              .withVisuallyHiddenText("newPensionCreditReference.change.hidden")
          )
        )
      )
    }
