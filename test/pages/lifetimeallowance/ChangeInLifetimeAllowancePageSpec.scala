...
class ChangeInLifetimeAllowancePageSpec extends PageBehaviours {
  ...
  "cleanup" - {
    ...
    "must cleanup correctly when user answers false" in {
      ...
      val cleanedUserAnswers = ChangeInLifetimeAllowancePage.cleanup(Some(false), ua).success.value
      ...
      cleanedUserAnswers.get(ChangeInLifetimeAllowancePage) mustBe Some(false)
    ...
    "must cleanup correctly when user answers true" in {
      ...
      val cleanedUserAnswers = ChangeInLifetimeAllowancePage.cleanup(Some(true), ua).success.value
