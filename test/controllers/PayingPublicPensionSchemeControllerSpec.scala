...
import forms.PayingPublicPensionSchemeFormProvider
...
import views.html.PayingPublicPensionSchemeView
...
class PayingPublicPensionSchemeControllerSpec extends SpecBase with MockitoSugar {
  ...
  lazy val NormalRoute = routes.PayingPublicPensionSchemeController.onPageLoad(NormalMode).url
  lazy val CheckRoute  = routes.PayingPublicPensionSchemeController.onPageLoad(CheckMode).url
  ...
  "PayingPublicPensionScheme Controller" - {
    ...
    "must redirect to StopPayingPublicPension when user answers false in NormalMode" in {
      ...
      running(application) {
        ...
        status(result) mustEqual SEE_OTHER
        redirectLocation(result).value mustEqual routes.StopPayingPublicPensionController.onPageLoad(NormalMode).url
      }
    }
    ...
    "must redirect to StopPayingPublicPension when user answers false in CheckMode" in {
      ...
      running(application) {
        ...
        status(result) mustEqual SEE_OTHER
        redirectLocation(result).value mustEqual routes.StopPayingPublicPensionController.onPageLoad(CheckMode).url
      }
    }
