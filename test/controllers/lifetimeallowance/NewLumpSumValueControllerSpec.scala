...
class NewLumpSumValueControllerSpec extends SpecBase with MockitoSugar {
  ...
  "NewLumpSumValue Controller" - {
    ...
    "must return OK and the correct view for a GET" in {
      ...
      running(application) {
        val request = FakeRequest(GET, newLumpSumValueRoute)

        val result = route(application, request).value

        val view = application.injector.instanceOf[NewLumpSumValueView]
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      val userAnswers = UserAnswers(userAnswersId).set(NewLumpSumValuePage, validAnswer).success.value
    ...
    "must redirect to the next page when a non-zero valid data is submitted and excess is lumpsum there is no old lumpsum value" in {
      ...
      running(application) {
        val request =
          FakeRequest(POST, newLumpSumValueRoute)
