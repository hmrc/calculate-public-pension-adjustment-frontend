...
class NewAnnualPaymentValueControllerSpec extends SpecBase with MockitoSugar {
  ...
  "NewAnnualPaymentValue Controller" - {
    ...
    "must return OK and the correct view for a GET" in {
      ...
      running(application) {
        val request = FakeRequest(GET, newAnnualPaymentValueRoute)

        val result = route(application, request).value

        val view = application.injector.instanceOf[NewAnnualPaymentValueView]
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      val userAnswers = UserAnswers(userAnswersId).set(NewAnnualPaymentValuePage, validAnswer).success.value
    ...
    "must redirect to the next page when submitted" in {
      ...
      running(application) {
        val request =
          FakeRequest(POST, newAnnualPaymentValueRoute)
