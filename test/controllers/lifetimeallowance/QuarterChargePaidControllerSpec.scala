...
import forms.lifetimeallowance.QuarterChargePaidFormProvider
import models.{Done, NormalMode, QuarterChargePaid, UserAnswers}
...
import views.html.lifetimeallowance.QuarterChargePaidView

import scala.concurrent.Future

class QuarterChargePaidControllerSpec extends SpecBase with MockitoSugar {
  ...
  "QuarterChargePaid Controller" - {
    ...
    "must return OK and the correct view for a GET" in {
      ...
      running(application) {
        val request = FakeRequest(GET, quarterChargePaidRoute)

        val result = route(application, request).value

        val view = application.injector.instanceOf[QuarterChargePaidView]
    ...
    "must redirect to the next page when valid data is submitted" in {
      ...
      running(application) {
        val request =
          FakeRequest(POST, quarterChargePaidRoute)
            .withFormUrlEncodedBody(("value", QuarterChargePaid.values.head.toString))
