...
class HowMuchContributionPensionSchemeControllerSpec extends SpecBase with MockitoSugar {
  ...
  lazy val howMuchContributionPensionSchemeRoute =
  ...
  "HowMuchContributionPensionScheme Controller" - {
    ...
    "must return OK and the correct view for a GET" in {
      ...
      running(application) {
        val request = FakeRequest(GET, howMuchContributionPensionSchemeRoute)
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      val userAnswers =
        UserAnswers(userAnswersId).set(HowMuchContributionPensionSchemePage(Period._2018), validAnswer).success.value
