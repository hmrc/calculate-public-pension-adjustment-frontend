...
import forms.annualallowance.taxyear.DoYouHaveGiftAidFormProvider
...
class DoYouHaveGiftAidControllerSpec extends SpecBase with MockitoSugar {

  def onwardRoute = Call("GET", "/foo")

  val startEndDate: String = "6 April 2021 to 5 April 2022"

  val formProvider = new DoYouHaveGiftAidFormProvider()
  val form         = formProvider(startEndDate)
  val period       = Period._2022

  lazy val doYouHaveGiftAidRoute =
    controllers.annualallowance.taxyear.routes.DoYouHaveGiftAidController.onPageLoad(NormalMode, period).url

  "DoYouHaveGiftAid Controller" - {
    ...
    "must return OK and the correct view for a GET" in {
      ...
      running(application) {
        val request = FakeRequest(GET, doYouHaveGiftAidRoute)
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      val userAnswers = UserAnswers(userAnswersId).set(DoYouHaveGiftAidPage(period), true).success.value
    ...
    "must redirect to the next page when valid data is submitted" in {
      ...
      running(application) {
        val request =
          FakeRequest(POST, doYouHaveGiftAidRoute)
