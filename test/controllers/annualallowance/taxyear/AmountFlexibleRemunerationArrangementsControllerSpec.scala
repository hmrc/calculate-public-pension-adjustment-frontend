...
class AmountFlexibleRemunerationArrangementsControllerSpec extends SpecBase with MockitoSugar {
  ...
  "AmountFlexibleRemunerationArrangements Controller" - {
    ...
    "must return OK and the correct view for a GET" in {
      ...
      running(application) {
        val request = FakeRequest(GET, amountFlexibleRemunerationArrangementsRoute)
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      val userAnswers =
        UserAnswers(userAnswersId)
          .set(AmountFlexibleRemunerationArrangementsPage(Period._2018), validAnswer)
          .success
          .value
    ...
    "must redirect to the next page when valid data is submitted" in {
      ...
      running(application) {
        val request =
          FakeRequest(POST, amountFlexibleRemunerationArrangementsRoute)
