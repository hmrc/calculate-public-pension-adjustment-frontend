...
class AmountSalarySacrificeArrangementsControllerSpec extends SpecBase with MockitoSugar {
  ...
  lazy val amountSalarySacrificeArrangementsRoute =
  ...
  "AmountSalarySacrificeArrangements Controller" - {
    ...
    "must return OK and the correct view for a GET" in {
      ...
      running(application) {
        val request = FakeRequest(GET, amountSalarySacrificeArrangementsRoute)
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      val userAnswers =
        UserAnswers(userAnswersId).set(AmountSalarySacrificeArrangementsPage(Period._2018), validAnswer).success.value
