...
import models.{NormalMode, PreviousClaimContinue, UserAnswers}
...
class PreviousClaimContinueControllerSpec extends SpecBase with MockitoSugar {
  ...
  "PreviousClaimContinue Controller" - {
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      val userAnswers = UserAnswers(userAnswersId).set(PreviousClaimContinuePage, PreviousClaimContinue.values.head).success.value
      ...
      running(application) {
        ...
        status(result) mustEqual OK
        contentAsString(result) mustEqual view(form.fill(PreviousClaimContinue.values.head), NormalMode)(request, messages(application)).toString
      }
    }
      ...
      running(application) {
        val request =
          FakeRequest(POST, previousClaimContinueRoute)
            .withFormUrlEncodedBody(("value", PreviousClaimContinue.values.head.toString))
