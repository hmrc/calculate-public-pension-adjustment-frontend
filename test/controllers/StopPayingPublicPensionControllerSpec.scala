...
import forms.StopPayingPublicPensionFormProvider
...
import views.html.StopPayingPublicPensionView
...
class StopPayingPublicPensionControllerSpec extends SpecBase with MockitoSugar {
  ...
  lazy val NormalRoute = routes.StopPayingPublicPensionController.onPageLoad(NormalMode).url
  lazy val CheckRoute  = routes.StopPayingPublicPensionController.onPageLoad(CheckMode).url

  override val emptyUserAnswers = UserAnswers(userAnswersId)

  def getRequest(mode: Mode): FakeRequest[AnyContentAsEmpty.type] =
    FakeRequest(GET, routes.StopPayingPublicPensionController.onPageLoad(mode).url)

  def postRequest(mode: Mode): FakeRequest[AnyContentAsFormUrlEncoded] =
    FakeRequest(POST, routes.StopPayingPublicPensionController.onPageLoad(mode).url)
