...
import forms.annualAllowance.setupQuestions.PayingPublicPensionSchemeFormProvider
...
import views.html.annualAllowance.setupQuestions.PayingPublicPensionSchemeView

import scala.concurrent.Future

class PayingPublicPensionSchemeControllerSpec extends SpecBase with MockitoSugar {
  ...
  lazy val NormalRoute = setupAARoutes.PayingPublicPensionSchemeController.onPageLoad(NormalMode).url
  lazy val CheckRoute  = setupAARoutes.PayingPublicPensionSchemeController.onPageLoad(CheckMode).url
  ...
  "PayingPublicPensionScheme Controller" - {
    ...
    "must return OK and the correct view for a GET" in {
      ...
      running(application) {
        ...
        val view = application.injector.instanceOf[PayingPublicPensionSchemeView]
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      val userAnswers = UserAnswers(userAnswersId).set(PayingPublicPensionSchemePage, true).success.value
    ...
    "must redirect to StopPayingPublicPension when user answers false in NormalMode" in {
      ...
      running(application) {
        ...
        status(result) mustEqual SEE_OTHER
        redirectLocation(result).value mustEqual setupAARoutes.StopPayingPublicPensionController.onPageLoad(NormalMode).url
      }
    }
    ...
    "must redirect to StopPayingPublicPension when user answers false in CheckMode" in {
      ...
      running(application) {
        ...
        status(result) mustEqual SEE_OTHER
        redirectLocation(result).value mustEqual setupAARoutes.StopPayingPublicPensionController.onPageLoad(CheckMode).url
      }
    }
