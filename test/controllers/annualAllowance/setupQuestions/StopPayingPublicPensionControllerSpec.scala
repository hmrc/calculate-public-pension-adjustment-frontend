...
import pages.annualAllowance.setupQuestions.StopPayingPublicPensionPage
import play.api.inject.bind
import play.api.mvc.{AnyContentAsEmpty, AnyContentAsFormUrlEncoded, Call}
import play.api.test.FakeRequest
import play.api.test.Helpers._
import repositories.SessionRepository
import views.html.annualAllowance.setupQuestions.StopPayingPublicPensionView
...
class StopPayingPublicPensionControllerSpec extends SpecBase with MockitoSugar {
  ...
  lazy val NormalRoute = setupAARoutes.StopPayingPublicPensionController.onPageLoad(NormalMode).url
  lazy val CheckRoute  = setupAARoutes.StopPayingPublicPensionController.onPageLoad(CheckMode).url

  override val emptyUserAnswers = UserAnswers(userAnswersId)

  def getRequest(mode: Mode): FakeRequest[AnyContentAsEmpty.type] =
    FakeRequest(GET, setupAARoutes.StopPayingPublicPensionController.onPageLoad(mode).url)

  def postRequest(mode: Mode): FakeRequest[AnyContentAsFormUrlEncoded] =
    FakeRequest(POST, setupAARoutes.StopPayingPublicPensionController.onPageLoad(mode).url)
