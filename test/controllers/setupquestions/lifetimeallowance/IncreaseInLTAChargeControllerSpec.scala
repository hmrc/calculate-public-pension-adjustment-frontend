...
import views.html.setupquestions.lifetimeallowance.IncreaseInLTAChargeView

import scala.concurrent.Future

class IncreaseInLTAChargeControllerSpec extends SpecBase with MockitoSugar {
  ...
  "IncreaseInLTACharge Controller" - {
    ...
    "must return OK and the correct view for a GET" in {
      ...
      running(application) {
        val request = FakeRequest(GET, increaseInLTAChargeRoute)

        val result = route(application, request).value

        val view = application.injector.instanceOf[IncreaseInLTAChargeView]
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      val userAnswers = UserAnswers(userAnswersId).set(IncreaseInLTAChargePage, true).success.value
