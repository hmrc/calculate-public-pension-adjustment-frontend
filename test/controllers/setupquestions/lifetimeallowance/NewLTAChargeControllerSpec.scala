...
import views.html.setupquestions.lifetimeallowance.NewLTAChargeView

import scala.concurrent.Future

class NewLTAChargeControllerSpec extends SpecBase with MockitoSugar {
  ...
  "NewLTACharge Controller" - {
    ...
    "must return OK and the correct view for a GET" in {
      ...
      running(application) {
        val request = FakeRequest(GET, newLTAChargeRoute)

        val result = route(application, request).value

        val view = application.injector.instanceOf[NewLTAChargeView]
    ...
    "must populate the view correctly on a GET when the question has previously been answered" in {
      ...
      val userAnswers = UserAnswers(userAnswersId).set(NewLTAChargePage, true).success.value
