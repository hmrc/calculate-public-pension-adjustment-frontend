  ...
  "UserAnswersConnector" - {
    ...
    "recordsPresentInSubmissionService" - {
      "must return a true when server returns future true" in {
        ...
        val url = s"/submit-public-pension-adjustment/check-submission-status/id"
        val app = subApplication
        ...
        running(app) {
          ...
          val result = connector.recordsPresentInSubmissionService("id")(hc).futureValue
      ...
      "must return a failed future when the server responds with BAD_REQUEST" in {
        ...
        running(app) {
          ...
          whenReady(connector.recordsPresentInSubmissionService("id")(hc).failed) { exc =>
